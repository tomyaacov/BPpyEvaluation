...
#SBATCH --job-name run_single_trace_search ### name of the job. replace my_job with your desired job name
#SBATCH --output run_single_trace_search.out ### output log for running job - %J is the job number variable
...
for option in "${options[@]}"; do
  ...
  for i in {1..10}
  do
    timeout 120m /usr/bin/time -a -o run_single_trace_search_output.csv -f "$option,$i,%E,%M" python pancake_single_trace_search.py $option
  done
done