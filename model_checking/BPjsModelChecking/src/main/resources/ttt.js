...
bp.registerBThread('EndOfGame', function () {
    bp.hot(true).sync({ waitFor: [StaticEvents.OWin, StaticEvents.XWin, StaticEvents.draw] })
...
bp.registerBThread('DetectDraw', function () {
    ...
    bp.sync({ request: [StaticEvents.draw] , block: move})
})
...
function addLineBThreads(xline, oline) {
    ...
    bp.registerBThread('DetectXWin', function () {
        ...
        bp.sync({ request: [StaticEvents.XWin] , block: move})
    })
    ...
    bp.registerBThread('DetectOWin', function () {
        ...
        bp.sync({ request: [StaticEvents.OWin] , block: move})
    })
...
bp.registerBThread('Simulate O', function () {
    var cells = []
    for (var i = 0; i < R; i++) {
        for (var j = 0; j < C; j++) {
            cells.push(O(i, j))
